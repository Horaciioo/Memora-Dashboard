...
export const GET = createProtectedRoute({
  ...
  handler: ({ query }) => readTeamBoard(query.get(SCOPE_PARAM) ?? undefined),
})
...
export const POST = createProtectedRoute({
  ...
  handler: async ({ raw, query }) => {
    ...
    return createTeam(parsed.values, query.get(SCOPE_PARAM) ?? undefined)
  },
})
...
export const PATCH = createProtectedRoute({
  ...
  handler: ({ raw, query }) => {
    ...
    return moveMember(accountId, teamId, query.get(SCOPE_PARAM) ?? undefined)
  },
})
